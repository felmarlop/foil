...
"""Creación de clases para predicados y reglas"""


class pred():
    variables = []
    """Constructor del predicado. Formado por un nombre y una lista de
    variables"""
    ...
    """Mostrar y Equals"""
    """Metodos para pintar el predicado por pantalla y para comparar
    con otro predicado"""
    def __repr__(self):
        return self.nPredicado+self.variables.__repr__()

    def __equals__(self, other):
        return (self.nPredicado, self.variables) == (other.nPredicado, other.variables)


class regla():
    cabecera = pred
    extensiones = []
    """Constructor de la regla. Formado por una cabecera, que es un predicado
    y una lista de predicados."""
    ...
    """Mostrar regla"""
    """Metodos para pintar la regla por pantalla y para comparar
    con otra regla"""
    def __repr__(self):
        if(len(self.extensiones) == 0):
            return self.cabecera.__repr__()+" :- "
        else:
            return self.cabecera.__repr__()+" :- "+self.extensiones.__repr__()
"""----------------------------------------------------------------------"""
"""Variables y Ganancia"""

"""Método que devuelve las variables usadas en la regla"""
...
Devuelve el valor de la ganancia de una regla"""
...
def getGanancia_informacion(t, p, n, ppr, npr):
    if (t == 0):
        ...
        res = round(t*(math.log(ppr/(ppr+npr), 2)-math.log(p/(p+n), 2)), 3)
    return res
"""-----------------------------------------------------------------------"""
"""Constantes, extender ejemplos y variables contenidas"""

"""Metodo que nos devuelve todas las constantes por separado y sin repetir que
hay en nuestro problema. Pertenecientes siempre a la base de conocimiento."""
...
def mundoCerrado(bc, p, ejP):
    l = []
    res = []
    l.extend(itertools.product(getConstantes(bc),repeat=len(p.getVariables())))
    for c in l:
        if list(c) not in ejP:
            res.append(list(c))
    return res

"""Metodo utilizado para saber si en la condicion hay alguna variable que no
está en la cabecera. Devuelve tambien en la segunda posicion de la lista, las
variables que están en la condicion y tambien en la cabecera. En el caso de que
existan varias extensiones o condiciones, comprueba si en la ultima extensión
hay variables que están en la cabecera o en el resto de extensiones,
devolviendo tambien esa lista de variables coincidentes."""
...
def getCubiertos_regla(r1, ej, constantes_Problema, bc):
    ...
    if len(r1.getExtensiones()) == 0:
        ...
        for i in range(len(ej)):
            ...
            if(contiene0 == 0):
                ...
                p1 = pred(r1.getExtensiones()[-1].getnPredicado(), constExtension)
            ...
            #caso 2:
                ...
                varNuevas = []
                #Creo la lista de variables nuevas que tenemos.
                ...
                for pr in bc:
                    ejCubierto = []
                    constPredicado = []
                    if(pr.getnPredicado() == r1.getExtensiones()[-1].getnPredicado()):
                        ...
                        for var in contiene1:
                            varUltima = r1.getExtensiones()[-1].getVariables()[:]
                            posiciones = [item for item in range(len(varUltima)) if varUltima[item] == var]
                        #Comprobamos si en las posiciones que hemos calculado en
                        #el predicado de la BC se encuentra la misma constante
                        #que la asignada a la variable por la que vamos 
                        #comprobando, registrada en "pares". Si es así, las 
                        #vamos eliminando hasta quedarnos solo con lo que valen  
                        #las variables nuevas de la condición que nos ha llegado.
                            for pos in posiciones:
                                if(pr.getVariables()[pos] == pares[var]):
                                    constPredicado.remove(pares[var])
                                else:
                                    break
                        #Por último, la lista de valores de las constantes nuevas
                        #la añadimos al ejemplo cubierto. Tratando además cuando
                        #existen variables repetidas.
                        if len(constPredicado) == len(varNuevas):
                            if len(set(constPredicado)) == len(set(varNuevas)):
                                nuevosPares = {}
                                flag = True
                                for ii in range(len(varNuevas)):
                                    if varNuevas[ii] not in nuevosPares:
                                        nuevosPares[varNuevas[ii]] = constPredicado[ii]
                                    else:
                                        if nuevosPares[varNuevas[ii]] != constPredicado[ii]:
                                            flag = False
                                if flag == True:
    ...
    return ejemplos
"""Método que nos devuelve todos los posibles predicados o literales que
podemos añadir al cuerpo de la regla"""
    ...
    for l in prn:
        ...
        for ad in range(len(l.getVariables())-1):
            if variables[-1][0] == "V" and len(variables[-1])>1:
                ...
                variables.append("V1")
        combinaciones.extend(itertools.product(variables,repeat=len(l.getVariables())))
...
def mejor(reg, pos, neg, ejP, ejN, constantes_Problema, bc):
    ganancia = 0
    res = []
...
def foil(ejemplosP, ejemplosN, bc, predObjetivo):
    ...
    reglas_Aprendidas = []
    #2.Hacer E igual a ejemplos. Creo algunas listas que me haran falta más
    #adelante para poder eliminar al final los positivos cubiertos finales.
    EP = []
    EN = []
    EP = ejemplosP
    EN = ejemplosN
    ...
    while EP:
        ...
        while len(negativos) > 0:
            #3.2.1 Generar todos los posibles literales que pueden ser añadidos
            #al cuerpo de la regla.
            #La lista de literales la crea dentro del metodo mejor()
            #3.2.2 De todos ellos sea L el mejor. Si no podemos calcular el
            #mejor, se mostrará un mensaje y saldremos del bucle interno.
            literalL = mejor(reglaIni, ejemplosP, ejemplosN, positivos,
                             negativos, constantes_Problema, bc)
            if(literalL == []):
                print("\nNo se puede obtener el siguiente mejor literal"+
                      " para la regla "+reglaIni.__repr__()+" con los ejemplos"+
                      " ofrecidos")
                break
            #3.2.3 Actualizar R añadiendo el literal L al cuerpo de R
            reglaIni.addExtension(literalL)
                #Calculo los nuevos negativos y positivos para sacar el
                #siguiente literal mejor.
            ...
            for i in positivos:
                ...
                ej = []
        #3.3 Incluir R en reglas aprendidas. Si el mejor literal no se calculó,
        #salimos del bucle externo.
        if(literalL == []):
            print("Regla aprendida: ", reglaIni.__repr__(), "\n\n")
            break
...
"""Métodos"""
"""Ganancia"""
"""Delvuel el valor de la ganancia nfoil de una regla. p:ejemplos positivos
iniciales. Corresponde al método score de nFOIL"""
...
def getGanancia_nFoil(t, p, n, ppr, npr):
    if (t == 0):
        ...
        res = round(((t/(p + n + npr))*(math.log(ppr/(ppr+npr), 2)-math.log(p/(p+n), 2))), 3)
    return res
"""Mejor Nfoil"""
"""Igual que el mejor de FOIL, pero en este caso la ganancia es distinta,
específica para nfoil. Devuelve, además del mejor literal, la ganancia o score
del mismo"""


def mejorNfoil(reg, pos, neg, ejP, ejN, constantes_Problema, bc):
    ganancia = 0
    res = []
    #Guardamos la lista de literales o predicados a partir del metodo genera.
    #Calculamos la lista de positivos y negativos cubiertos inicialmente.
    ...
    return resFin
"""nFoil"""
"""Método muy parecido al foil, sin embargo en este caso utilizamos el
...
def nFoil(ejemplosP, ejemplosN, bc, predObjetivo):
    ...
    while gananciaAct > umbral:
        ...
        while len(negativos) > 0:
            ...
            #3.2.2 De todos ellos sea L el mejor. Si no podemos calcular el
            #mejor, se mostrará un mensaje y saldremos del bucle interno.
            mejor = mejorNfoil(reglaIni, ejemplosP, ejemplosN, positivos,
                               negativos, constantes_Problema, bc)
            literalL = mejor[0]
            gananciaAct = mejor[1]
            if(literalL == []):
                print("\nError: No se puede obtener el siguiente mejor literal"+
                      " para la regla "+reglaIni.__repr__()+" con los ejemplos"+
                      " ofrecidos")
                break
            #3.2.3 Actualizar R añadiendo el literal L al cuerpo de R
